...
public:
    ...
    {
        // Create a dedicated engine for our test
        te::Engine engine("PsgTrackTest");
        testDirectInjection(engine);
        testWorkaroundApproach(engine);
    }

private:
    void testDirectInjection(te::Engine& engine)
    {
        ...
        psgTrackState.setProperty(te::IDs::id, te::EditItemID::createNewID(edit->getProjectItemID()).toString(), nullptr);
        
        // Add the track directly to the edit state
        edit->getTrackList().state.addChild(psgTrackState, -1, nullptr);
    ...
    void testWorkaroundApproach(te::Engine& engine)
    {
        ...
        logMessage("As expected, direct track creation with PSGTRACK fails");
        ...
        logMessage("AudioTrack created successfully with state:\n" + 
                   audioTrack->state.toXmlString());
            
        logMessage("Step 3: Copy AudioTrack state and convert to PSGTRACK");
        juce::ValueTree psgState = audioTrack->state.createCopy();
        psgState.setType(IDs::PSGTRACK);
        ...
        edit->getTrackList().state.addChild(psgState, originalPosition, &edit->getUndoManager());
