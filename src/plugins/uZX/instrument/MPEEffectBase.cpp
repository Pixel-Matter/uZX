#include "MPEEffectBase.h"

namespace MoTool::uZX {

MPEEffectBase::MPEEffectBase() : mpeInstrument(defaultInstrument) {
    mpeInstrument.addListener(this);
}

MPEEffectBase::MPEEffectBase(MPEInstrument& inst) : mpeInstrument(inst) {
    mpeInstrument.addListener(this);
}

//==============================================================================
MPEZoneLayout MPEEffectBase::getZoneLayout() const noexcept {
    return mpeInstrument.getZoneLayout();
}

void MPEEffectBase::setZoneLayout(MPEZoneLayout newLayout) {
    mpeInstrument.setZoneLayout(newLayout);
}

//==============================================================================
void MPEEffectBase::enableLegacyMode(int pitchbendRange, Range<int> channelRange) {
    mpeInstrument.enableLegacyMode(pitchbendRange, channelRange);
}

bool MPEEffectBase::isLegacyModeEnabled() const noexcept {
    return mpeInstrument.isLegacyModeEnabled();
}

Range<int> MPEEffectBase::getLegacyModeChannelRange() const noexcept {
    return mpeInstrument.getLegacyModeChannelRange();
}

void MPEEffectBase::setLegacyModeChannelRange(Range<int> channelRange) {
    mpeInstrument.setLegacyModeChannelRange(channelRange);
}

int MPEEffectBase::getLegacyModePitchbendRange() const noexcept {
    return mpeInstrument.getLegacyModePitchbendRange();
}

void MPEEffectBase::setLegacyModePitchbendRange(int pitchbendRange) {
    mpeInstrument.setLegacyModePitchbendRange(pitchbendRange);
}

//==============================================================================
void MPEEffectBase::setPressureTrackingMode(TrackingMode modeToUse) {
    mpeInstrument.setPressureTrackingMode(modeToUse);
}

void MPEEffectBase::setPitchbendTrackingMode(TrackingMode modeToUse) {
    mpeInstrument.setPitchbendTrackingMode(modeToUse);
}

void MPEEffectBase::setTimbreTrackingMode(TrackingMode modeToUse) {
    mpeInstrument.setTimbreTrackingMode(modeToUse);
}

//==============================================================================
void MPEEffectBase::handleMidiEvent(const MidiMessage& m) {
    mpeInstrument.processNextMidiEvent(m);
}

//==============================================================================
void MPEEffectBase::renderNextBlock(const MidiBuffer& inputMidi,
    ...
    for (auto it = inputMidi.findNextSamplePosition(startSample); it != inputMidi.cend(); ++it) {
        ...
        if (metadata.samplePosition >= prevSample + thisBlockSize) {
            renderNextSubBlock(outputMidi, prevSample, metadata.samplePosition - prevSample);
    ...
    if (prevSample < endSample)
        renderNextSubBlock(outputMidi, prevSample, endSample - prevSample);
}

//==============================================================================
void MPEEffectBase::setCurrentPlaybackSampleRate(const double newRate) {
...
//==============================================================================
void MPEEffectBase::setMinimumRenderingSubdivisionSize(int numSamples, bool shouldBeStrict) noexcept {
...
class MpeSynthesiserBaseTests final : public UnitTest
{
    ...
    class MockSynthesiser final : public MPEEffectBase
    {
    public:
        Events events;
    ...
    private:
        using MPEEffectBase::renderNextSubBlock;

        void renderNextSubBlock(AudioBuffer<float>&, int startSample, int numSamples) override {
...
public:
    MpeSynthesiserBaseTests() : UnitTest("MPE Synthesiser Base", UnitTestCategories::midi) {}
    ...
    void runTest() override {
        ...
        beginTest("Rendering sparse subblocks works");
        {
...

            {
                ...
                expect(e.order
                       == std::vector<CallbackKind>{CallbackKind::process, CallbackKind::midi, CallbackKind::process});
            }
        }
            ...
            for (auto strict : {false, true}) {
                for (auto subblockSize : {1, 16, 32, 64, 1024}) {
                    ...
                    expectWithinAbsoluteError(
                        float(e.blocks.size()), std::ceil((float) blockSize / (float) subblockSize), 1.0f);
                    expect(e.messages.size() == blockSize);
                    expect(std::is_sorted(e.blocks.begin(), e.blocks.end()));
                    expect(sumBlockLengths(e.blocks) == blockSize);
